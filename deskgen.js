...
class DeskSchedule {
            ...
            if (wiwUserObj != undefined) {
                ...
                gCalEvents.forEach(gCalEvent => {
                    ...
                    if (guestEmailList.some(guestEmail => guestEmail.localeCompare(wiwUserObj.email, "en", { sensitivity: "base" }) === 0)) {
                        ...
                        eventsFormatted.push(new ShiftEvent(gCalEvent.getTitle(), startTime, endTime, this.getEventUrl(gCalEvent)));
                    }
                });
    ...
    updateFloorOverstaffCount(timeStart, timeEnd, useLowMinimums = true, halfHourGrace = true) {
        ...
        this.floors.forEach((floor) => {
            ...
            if (halfHourGrace && !useLowMinimums && lowTimes.length == 1) {
                //TODO: OR if length is < 4 and non adjacent (min difference between elements in array)
                //TODO: halfHourGrace shouldn't be used if there is only one person on floor - Jul 26?
                //if 
    ...
    timelineBalanceFloors(balanceTimeStart = this.settings.openTime(this.date), balanceTimeEnd = this.settings.openTime(this.date), useLowMinimums = true, initialBalance = true) {
        ...
        for (let i = 0; i < 20 &&
            (!useLowMinimums && (maxDifference(this.floors.map(floor => floor.overstaffMin)) > 1)
                ...
                || (useLowMinimums && this.floors.some(floor => floor.overstaffMin < 0))); i++) { //or if ASRS is > other stations?
                ...
                if (!useLowMinimums || takingFloor.overstaffMin < 0) {
                    ...
                    for (const givingShift of givingFloorShifts) {
                        if ((!useLowMinimums && givingFloor.overstaffMin - takingFloor.overstaffMin > 1)
                            ...
                            if (initialBalance /*|| givingShift.startTime == time.getTime()*/)
    ...
    timelineBreakUpASRS() {
        ...
        longAsrsShifts.forEach(shiftToSplit => {
            ...
            //if moving half of shift isn't need to balance floors, try to swap with another shift that needs splitting
                ...
                console.log(shiftToSplit.name + ": can't find any shifts up to Specialists to swap with");
        });
    }
    trySwap(shiftToSplit, shiftsToSwap) {
    ...
    timelineAddStations(stations, shifts, assignMeals = true, balanceFloors = true) {
        ...
        for (let time = new Date(startTime); time < endTime; time.addTime(0, 30)) {
            ...
            for (const floor of this.floors) {
                let floorShifts = shifts.filter(shift => shift.floor?.name == floor.name && shift.getStationAtTime(time).name == this.defaultStations.undefined.name);
                if (stations.some(station => station.name == "HOP"))
                    floorShifts = shifts;
                let floorStations = stations.filter(station => {
                    let onFloor = (station.floor == floor.name || station.floor == undefined);
                    let notDefault = (!Object.values(this.defaultStations).map(s => s.name).includes(station.name) || station.name == this.defaultStations.available.name);
                    let validTime = this.stationIsWithinTimeLimits(station, time);
                    let notOverStaffLimit = this.getStationCountAtTime(this.shifts, station, time) < station.numOfStaff; //could this break with multiple stations of same name?
                    return onFloor && notDefault && validTime && notOverStaffLimit;
                });
                if (floorShifts.length <= 0 || floorStations.length <= 0)
                    continue;
                ...
                const adjustRating = (shiftIndex, stationIndex, description, value) => {
                ...
                for (const [shiftIndex, shift] of floorShifts.entries()) {
                    if (shift.getStationAtTime(time).name == this.defaultStations.off.name) {
                        floorStations.forEach((s, stationIndex) => adjustRating(shiftIndex, stationIndex, "off", -1000000));
                        continue;
                    }
                    ...
                    for (const [stationIndex, station] of floorStations.entries()) {
                        if (!this.assignmentEligibilityCheckShift(shift, station, time)) {
                ...
                /*
                          ...
                          floorStations.forEach((station, stationIndex)=>{
                            ...
                            floorShifts.forEach(shift=> {
                              if (station.positionPriority.length<1 || station.positionPriority.some(pos=>
                                ...
                                let nextStation = shift.getStationAtTime(nextTime)
                                let prevStation = shift.getStationAtTime(prevTime)
                                let undefinedCount = this.getStationCountAtTime(floorShifts, this.defaultStations.undefined, time, floor.name)
                        ...
                        floorStations.forEach((station,stationIndex)=>{
                          ...
                          if (this.settings.locationID == 5786790)
                            this.sortShiftsByFairRotation(floorShifts)
                          else
                            this.sortShiftsByUserPositionPriority(floorShifts, station.positionPriority)
                          ...
                          // if (time.getTimeStringHHMM24()=="14:00" && station.name=="1st childrens'") console.log(`if not on station and over max move to front:\n${time.getTimeStringHHMM12()}, ${station.name}: ${floorShifts.map(s=>s.name).join('\n')}`)
                          ...
                          // if (time.getTimeStringHHMM24()=="13:00" && station.name=="Phones") console.log(`if available for this time and following half hour, move to front:\n${time.getTimeStringHHMM12()}, ${station.name}: ${floorShifts.map(s=>`${s.name}, now:${s.getStationAtTime(time).name}, next:${s.getStationAtTime(nextTime).name}, ${s.getStationAtTime(time).name==this.defaultStations.undefined}, ${s.getStationAtTime(nextTime).name==this.defaultStations.undefined}`).join('\n')}`)
                          ...
                          if (this.settings.locationID == 5786790 && station.name.includes("Do Space")) floorShifts.sort((shiftA, shiftB)=>{
                            let bVal = shiftB.tags.includes("Do Space") ? -1 : 0
                            let aVal = shiftA.tags.includes("Do Space") ? -1 : 0
                            return aVal-bVal
                          })
                ...
                */
                ...
                function makeTableHTML(myArray, cssclass = '') {
    ...
    assignmentEligibilityCheckShift(shift, station, time) {
        if (station.positionPriority.length < 1 || station.positionPriority.some(pos => pos.enabled && pos.title == this.getPositionById(shift.position)?.name)) { /*continue*/ }
        else
            return false;
        let currentStation = shift.getStationAtTime(time);
        ...
        // if(time.getTimeStringHHMM24() == "18:00")
        //   this.ui.alert(`
        //   ${time.getTimeStringHHMM24()}    ${shift.name} - ${station.name}
        //   eligible position: ${station.positionPriority.length<1 || station.positionPriority.some(pos=>
        //   pos.enabled && pos.title == this.getPositionById(shift.position)?.name)}
        //   currently undefined: ${currentStation.name == this.defaultStations.undefined.name}
        //   not in training: ${!shift.tags.includes("In training (do not assign to stations)")}
        //   numOfStaff not over max: ${stationCount < station.numOfStaff}`)
        ...
        currentStation.name == this.defaultStations.undefined.name
            ...
            return false;
        //ForXhoursperdayforeachstaff
        if (station.durationType == DurationType.ForXhoursperdayforeachstaff && this.getTotalStationCount(shift, station.name, time) >= station.duration)
            return false;
        return true;
    }
    stationIsWithinTimeLimits(station, time) {
        ...
        if (station.durationType == DurationType.ForXhoursperdayforeachstaff
            ...
            && this.shifts.some(shift => this.getTotalStationCount(shift, station.name, time) < station.duration))
            return true;
        return false;
    }
    assignmentEligibilityCheck(shift, station, time) {
        ...
        currentStation.name == this.defaultStations.undefined.name
            ...
            && !(station.name == "Tech Desk" && !shift.tags.includes("Do Space"))) { /*continue*/ }
        else
            return false;
        let withinLimit = false;
    ...
    displayTimeline() {
        ...
        this.floors.forEach((floor, i) => {
            ...
            let noteArr = floorShifts.map(shift => shift.noteTimeline);
    ...
    logDeskData(description, description2 = " ") {
...
class Station {
    constructor(color = "#ffffff", name, numOfStaff = 1, floor = undefined, positionPriority = [], durationType = DurationType.Alwayswhileopen, duration = 100, limitType = LimitType.SpecificTime, limitXHours = undefined, limitYHours = undefined, limitToStartTime = undefined, limitToEndTime = undefined, ratings = []) {
        this.color = color;
        this.name = name;
        this.floor = floor;
...
class ShiftEvent {
    constructor(title, startTime, endTime, gCalUrl) {
        this.title = title;
...
class Shift {
    ...
    countTotalTimeAtStation(stationName, beforeTime = this.deskSchedule.dayEndTime, afterTime = this.deskSchedule.dayStartTime) {
        let count = 0;
        ...
        if (values[row][0] == (values[row + 1] || [undefined])[0]) { }
            ...
            if (startRow < row) {
                ...
                if (row - startRow < 3 && r.getValue().length > 3)
                    r.setValue(rval == "Aide" ? rval : rval.substring(0, 3));
            }
 ...
 */
var MAX_SIZE = Number.MAX_SAFE_INTEGER / 2 || ((1 << 26) * (1 << 26));
/**
